...
def TrainEval(dataset, Recmodel, epoch, w=None, multicore=0):
    """
    Train set에 대해 추천 성능(Recall / Precision / NDCG / Hit Ratio)을 평가한다.
    ground truth로는 각 유저의 train positive 아이템(allPos)을 사용하며,
    Valid/Test와 달리 train 아이템을 마스킹하지 않고 전체 아이템에 대해 순위를 평가한다.
    """
    u_batch_size = world.config['test_u_batch_size']
    dataset: utils.BasicDataset
    Recmodel: model.LightGCN
    ...
    allPos = dataset.allPos  # 각 유저별 train positive 아이템 리스트
    ...
    with torch.no_grad():
        # train 상에서 적어도 하나의 positive를 가진 유저만 평가
        users = [u for u in range(len(allPos)) if len(allPos[u]) > 0]
        try:
            assert u_batch_size <= len(users) / 10
        except AssertionError:
            print(f"test_u_batch_size is too big for this dataset, try a small one {len(users) // 10}")
        ...
        total_batch = len(users) // u_batch_size + 1
        for batch_users in utils.minibatch(users, batch_size=u_batch_size):
            groundTrue = [allPos[u] for u in batch_users]  # train positives가 ground truth
            ...
            # TrainEval에서는 train 아이템을 마스킹하지 않음 (과적합/학습 적합도 확인용)
            _, rating_K = torch.topk(rating, k=max_K)
            rating = rating.cpu().numpy()
        ...
        if multicore == 1:
            pool.close()
        print("[TrainEval results]", results)
        return results
